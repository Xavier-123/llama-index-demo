...
class RequestModel(BaseModel):
    ...
    reranker_top: int = Field(3, example="", description="获取重排序的top节点数据")
...
async def api_build_embedding(embeding_retriever_list: list, reranker_list: list):
    ...
    if cfg["RERANKER_MODEL"]:
        ...
        reranker_list.append(SentenceTransformerRerank(model=cfg["RERANKER_MODEL"], top_n=5))


    return embeding_retriever_list, reranker_list
...
#         path=storepath
#     )
#     await client.add(collection_name=cfg["COLLECTION_NAME"], documents="111111111")




if __name__ == '__main__':
    asyncio.run(api_build_embedding(embeding_retriever_list, reranker_list))
