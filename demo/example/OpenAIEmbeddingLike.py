...
import httpx
import requests
from llama_index.core.embeddings.loading import OpenAIEmbedding
...
'''仿写openai接口的embedding类，可调远程部署的embedding模型'''
...
class OpenAIEmbeddingLike(OpenAIEmbedding):
    ...
    def _embed(
        ...
        response = requests.post("http://127.0.0.1:6008/v1/embeddings", json=data, headers=headers)

        s = response.json()
        req = s["data"][0]["embedding"]
        # print(len(req), req)
        return req
